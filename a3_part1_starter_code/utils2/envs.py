...
def play_episode_rb(env, policy, buf):
    ...
    while not done:
        ...
        # 
        # Note: done is a boolean, convert to float for buffer
        episode_trace.append((states[-1], action, reward, obs, float(done)))
        
        states.append(obs)
        rewards.append(reward)
    
    # 
    buf.add(episode_trace)
