...
本儲存庫包含了 Deep Recurrent Q-Learning (DRQN) 於部分可觀測 CartPole 環境，以及 Categorical DQN (C51) 於雜訊 CartPole 環境的實作程式碼與結果報告。

## 第一部分：深度遞歸 Q 學習 (Deep Recurrent Q-Learning, DRQN)
...
### 1.2 DQN 基準線 (問題所在)
我們首先訓練了一個標準 DQN 代理人作為基準線 (Baseline)。該網路僅由全連接層組成，且只接收當前的觀測幀。

* **觀測問題：** 在缺乏速度資訊的情況下，單一時間點的快照是具有歧義的。代理人無法區分桿子是靜止的還是正在移動的。
...
### 1.3 DRQN 實作細節
為了克服部分可觀測性，我們修改了網路結構以包含遞歸機制：
* **架構：** `Input -> Linear -> ReLU -> LSTM -> Linear -> Output`。
* **機制：** LSTM 維護一個隱藏狀態 ($h_t, c_t$)，能夠聚合時間序列上的資訊。這使得網路能夠從過往的觀測歷史中，隱式地推斷出缺失的速度與動量資訊。
* **訓練：** 我們使用了遞歸經驗回放緩衝區 (Recurrent Replay Buffer) 來採樣連續的序列 (Traces)，而非獨立的轉換 (Transitions)。
...
**LSTM 層的影響：**
結果清楚地展示了 LSTM 層的有效性。與 DQN 基準線不同，DRQN 代理人成功解決了任務，平均獎勵達到 **150 至 175** 之間 (接近最大值 200)。

**結論：**
標準的前饋網路 (DQN) 在 POMDPs 中失效，是因為 $O_t \neq S_t$。遞歸架構允許代理人整合時間資訊，有效地重建控制所需的底層狀態特徵 (如速度)。
...
* **分析：** 在約第 250-300 集 (Episodes) 時，效能出現明顯下滑。巨大的變異數 (陰影區域) 表明，在這個充滿雜訊的環境中，僅估計 *期望* 回報 (純量平均值) 不足以進行穩定的學習。
...
* **投影 (Projection)：** 我們實作了分佈式貝爾曼更新 (Distributional Bellman Update)，使用線性插值 (Lillicrap's projection) 將目標分佈投影到固定的支撐原子上，並最小化交叉熵損失 (Cross-Entropy Loss)。
...
2.  **魯棒性：** 透過學習回報的完整分佈而非僅是平均值，C51 捕捉了環境雜訊固有的變異與多模態特性。這提供了更豐富的學習訊號，使代理人能更有效地處理隨機動力學 (雜訊與摩擦)。

### 2.5 結論
結果表明，在隨機環境中，分佈式強化學習 (C51) 相較於純量價值估計 (DQN) 具有實質優勢，提供了改進的穩定性與魯棒性。
